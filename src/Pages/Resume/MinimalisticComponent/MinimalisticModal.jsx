...
export default function Minimalisticmodal({ selectedFileUrl, onClose }) {
  ...
  return (
    <AnimatePresence>
      {selectedFileUrl && (
        <motion.div
          className="fixed inset-0 bg-teal/60 flex justify-center items-center z-50"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-teal-500 hover:text-teal-800 z-10"
            >
              <X className="w-6 h-6" />
            </button>
